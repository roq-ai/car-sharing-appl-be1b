...
import { getIssues } from 'apiSdk/issues';
import { IssueInterface } from 'interfaces/issue';

import { BookingListPage as ShowCaseList } from 'pages/bookings';

function DashboardPage() {
  const { session } = useSession();

  const carStat = useSWR<PaginatedInterface<CarInterface>>(
    () => `/cars`,
    () => getCars({ limit: 1, offset: 0 }),
  );
  ...
  const issueStat = useSWR<PaginatedInterface<IssueInterface>>(
    () => `/issues`,
    () => getIssues({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Cars',
      result: carStat?.data?.totalCount,
      isLoading: carStat.isLoading,
    ...
    {
      title: 'Issues',
      result: issueStat?.data?.totalCount,
      isLoading: issueStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`bookings`} passHref legacyBehavior>
                  <Button
