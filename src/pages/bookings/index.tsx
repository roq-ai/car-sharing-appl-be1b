...
export function BookingListPage(props: BookingListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getBookings({
        relations: ['car', 'user', 'issue.count', 'review.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['pickup_location.contains', 'dropoff_location.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<BookingInterface>>(
    () => `/bookings?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const columns: ColumnType[] = [
    ...
    { id: 'pickup_location', header: 'Pickup Location', accessorKey: 'pickup_location' },
    { id: 'dropoff_location', header: 'Dropoff Location', accessorKey: 'dropoff_location' },
    ...
    hasAccess('issue', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          ...
          header: 'Issue',
          accessorKey: 'issue',
          cell: ({ row: { original: record } }: any) => record?._count?.issue,
        }
      : null,
