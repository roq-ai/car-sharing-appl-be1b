...
import { getIssueById } from 'apiSdk/issues';
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('issue', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/issues/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Issue Type" text={data?.issue_type} />
                ...
                <FormListItem
                  label="Reported Time"
                  text={
                    data?.reported_time ? format(parseISO(data?.reported_time as unknown as string), 'dd-MM-yyyy') : ''
                  }
                />
                ...
                <FormListItem
                  label="Resolved Time"
                  text={
                    data?.resolved_time ? format(parseISO(data?.resolved_time as unknown as string), 'dd-MM-yyyy') : ''
                  }
                />
                ...
                {hasAccess('booking', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Booking"
                    text={
                      <Link as={NextLink} href={`/bookings/view/${data?.booking?.id}`}>
                        {data?.booking?.pickup_location}
                      </Link>
                    }
                  />
                )}
