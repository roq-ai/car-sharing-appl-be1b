...
type ColumnType = ColumnDef<ReviewInterface, unknown>;
...
export function ReviewListPage(props: ReviewListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const fetcher = useCallback(
    async () =>
      getReviews({
        ...
        order: params.order,
        searchTermKeys: ['comment.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<ReviewInterface>>(
    () => `/reviews?params=${JSON.stringify(params)}`,
    fetcher,
  );
